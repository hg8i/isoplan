...
class editDialog:
    def __init__(self,window,setup,day,data):
        self._setup = setup
        self._day = day
        self._data = dict(data)
        self._origData = dict(data)
        self._window = window
        self._optionFocusColor     = setup["colors"]["dialogFocus"]
        self._optionRegularColor   = setup["colors"]["dialogBackground"]
        self._optionBackground     = setup["colors"]["dialogBackground"]
    ...
    def _drawTextline(self,y,x,name,defaultValue,focused):
        ...
        _text(self._window,y,x,name,color=color)
    ...
    def _drawData(self):
        ...
        for iKey, key in enumerate(inputsToShow):
            ...
            if yPos+2>self._screenY:
                ...
                _text(self._window,yPos+0,int((self._screenX-len(keyLine))/2),keyLine, color=self._optionRegularColor)
                break
...
def mainTest(screen):
    ...
    subScreen = screen.subwin(30,60,2,3)
    dialog = editDialog(subScreen,day,data[0])


    # # colors
    # curses.init_pair(20, 0, 15)
    # curses.init_pair(21, 1, 17)
    # curses.init_pair(1, 1, 1)
    # curses.init_pair(22, 0, 15)
    # curses.init_pair(22, 0, 15)
    curses.start_color()
